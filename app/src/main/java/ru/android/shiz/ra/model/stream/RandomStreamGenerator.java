...
public class RandomStreamGenerator implements StreamGenerator {
    ...
    public List<Stream> generateStreams() {
        ...
        int id = 0;
        int streamSize = 50;
        List<Stream> streams = new ArrayList<>(streamSize);

        long day = 24 * 60 * 60 * 1000;
        long timestamp = System.currentTimeMillis();

        for (int i = 0; i < streamSize; i++, timestamp -= day) {
    ...
    */
    ...
    @Override
    public Stream generateResponseStream(String senderStream) {

        String quotes[] = null;
        Person sender = null;

        if (senderStream.equals(Person.BARNEY.getEstream())) {
            sender = Person.BARNEY;
            quotes = barneyQuotes;
        } else if (senderStream.equals(Person.LILY.getEstream())) {
            quotes = lilyQuotes;
            sender = Person.LILY;
        } else if (senderStream.equals(Person.MARSHALL.getEstream())) {
            quotes = marshallQuotes;
            sender = Person.MARSHALL;
        } else if (senderStream.equals(Person.ROBIN.getEstream())) {
            quotes = robinQuotes;
            sender = Person.ROBIN;
        } else {
            return null;
