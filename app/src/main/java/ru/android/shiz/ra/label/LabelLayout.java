...
import ru.android.shiz.ra.utils.DimensUtils;
...
public class LabelLayout extends MvpViewStateLinearLayout<LabelView, LabelPresenter>
    ...
    private void init() {
        ...
        popUpWindow.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!label.getName().equals(stream.getLabel())) {
                    presenter.setLabel(stream, label.getName());
                    popUpWindow.dismiss();
                }
            }
        });
    ...
    public void setMail(Stream mail) {
        this.stream = stream;
        labelView.setText(stream.getLabel());
    }
    ...
    @Override
    public void changeLabel(Stream s, String label) {
        if (s.getId() == this.stream.getId()) {
            stream.label(label);
